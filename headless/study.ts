...
function report(
  ...
  L('  and digging where she rests, both invented, hold depth within about a third of it.')
  L('  After five years nests run about 1.2 to 1.3 times the law.')
  ...
  L('- **Colonies do not hold their size once grown.** The seed density on the ground is')
  L('  invented and fitted so that two colonies in three pass 700 workers, the size that')
  L('  rears queens and males, by their fifth year; the laying rate is tuned too. A colony')
  L('  that passes it falls back the year after, one from 1080 workers to 81, where real ones hold')
  L('  about 4300. Colony size, brood and alate output from this build are not the')
  L('  species’. See docs/DECISIONS.md D43.')
  L('- **Foraging peaks late.** The share foraging begins in April and rises through spring,')
  L('  but peaks in July where real colonies peak in May and June, and March is about zero.')
  L('- **Relocation has no cost and no choice of site.** Moves happen at the measured rate and')
  L('  distance, the store and brood are carried and the new nest is dug, but moving costs a')
  L('  colony nothing, and charcoal is not carried.')
  L('- **Alarm and drought are coarse.** The alarm scent is shorter-lived and shorter-ranged')
  L('  than one step, so how near a forager must be to respond is invented. How often')
