...
function report(
  ...
  L('  what the store yields. Colonies level off at 250 to 330 workers in their fourth and')
  L('  fifth years, where real colonies mature at about 700, so no colony reaches the size')
  L('  that rears queens and males. The seed density on the ground is invented and was not')
  L('  tuned, and the laying rate is tuned. Colony size, brood and alate output from this')
  L('  build are not the species’. See docs/DECISIONS.md D39.')
  L('- **Foraging starts too late in spring.** The share foraging peaks in June as measured,')
  L('  but stays near zero in March and April.')
  L('- **Relocation is abstracted.** Moves happen at the measured rate and distance, and the')
  L('  new nest is dug, but the walk between sites is not simulated and moving has no cost.')
